...
class CommandDelayReset(source: Source) : Command("", source) {
    ...
    override fun place(generator: Generator,
                       ...
                       face: Face) {
        if (face === Face.EAST) {
            blocks[x][y][z] = BlockCommand("/blockdata ~-1 ~1 ~ {Items:[]}",
                    EMPTY_STRING, face, CommandType.CHAIN, true, true)
        } else if (face === Face.WEST) {
            blocks[x][y][z] = BlockCommand("/blockdata ~1 ~1 ~ {Items:[]}",
                    EMPTY_STRING, face, CommandType.CHAIN, true, true)
        } else {
            throw GeneratorException(source,
                    "Reset placed in invalid direction: " + face)
        }
    }
