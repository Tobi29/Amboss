...
import org.tobi29.amboss.util.block.BlockFiller
import org.tobi29.amboss.util.block.BlockSign
...
class CommandActivator(private val name: String, source: Source) : Command("",
        source) {
    ...
    override fun place(generator: Generator,
                       ...
                       z: Int,
                       face: Face) {
        blocks[x][y][z] = BlockFiller("minecraft:stone", 0)
        blocks[x][y + 1][z] = BlockCommand(
                "/fill ~ ~-1 ~ ~ ~-1 ~ minecraft:stone 0 minecraft:redstone_block 0",
                Face.EAST, CommandType.REPEATING, true, false)
        blocks[x][y + 2][z] = BlockSign("{\"text\":\"Row:\"}",
                "{\"text\":\"$name\"}", Face.NONE)
    }
}
