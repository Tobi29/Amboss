...
class CommandDelayWait(source: Source) : Command("", source) {

    @Throws(GeneratorException::class)
    override fun eval(generator: Generator): Pair<String, Array<String>> {
        return Pair(
                "/testforblock ~ ~1 ~ minecraft:hopper 0 {Items:[{id:\"minecraft:iron_sword\"}]}",
    ...
    override fun place(generator: Generator,
        ...
        blocks[x][y][z] = BlockCommand(eval.first, eval.second, face,
                CommandType.REPEATING, true, false)
        blocks[x][y + 1][z] = BlockHopper(Face.DOWN)
    }
}
